...
class MinutesView {
    function __construct($pageData, $viewData) {        
        if($viewData->getType() === 'minutes-view') {
			...
			if(!file_exists($path)) {
	            ...
	            $documentURLS = array_map(function($e) {
	                ...
	                return array( 'name' => $e, 
	                              'url'  => SITEROOT . '/static/data/minutes/' . $dateName . '/' . $e);
	             }, $webFiles);
	            
	            if($directory->hasFile('minutes.xhtml')) {
	                $data = implode("", 
	                    file(FILEROOT . '/static/data/minutes/' . $dateName . '/minutes.xhtml'));
